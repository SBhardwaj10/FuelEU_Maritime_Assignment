...
export function allocatePool(members: PoolMember[]): PoolMember[] {
  // clone
  ...
  while (i < j) {
    if (m[i].cb_before <= 0) { i++; continue; }
    if (m[j].cb_before >= 0) { j--; continue; }

    const transfer = Math.min(m[i].cb_before, -m[j].cb_before);
    m[i].cb_before -= transfer;
    m[j].cb_before += transfer;
    if (m[i].cb_after === undefined) m[i].cb_after = 0;
    if (m[j].cb_after === undefined) m[j].cb_after = 0;
    // We will set cb_after at the end
    if (m[i].cb_before <= 0) i++;
    if (m[j].cb_before >= 0) j--;
  }
